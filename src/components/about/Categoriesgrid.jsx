...
const CategoryGrid = () => {
    ...
    return (
        <div className='max-w-[1240px] mx-auto'>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:gap-6 max-w-[350px] md:max-w-[1200px] mx-auto xl:gap-8 mt-24">
                {Object.entries(seriesNames).map(([key, series]) => (
                    <div
                        key={key}
                        onClick={() => handleCardClick(series._id)}
                        className="group relative flex flex-col items-center justify-end overflow-hidden border-2 border-white rounded-lg bg-white shadow-2xl col-span-1 h-52 md:h-60 cursor-pointer"
                    >
                        <img
                            src={images[key]}
                        ...
                        />
                        <div className="absolute bottom-0 left-0 right-0 bg-prime w-full z-40 py-2 flex flex-col items-center justify-center transition-all duration-400 ease-in-out group-hover:py-6">
