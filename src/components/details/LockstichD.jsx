...
const footerContent = {
  lockstitch: {
    ...
    description: {
      ...
      zh: "新的锁缝金星机器系列具有吸引人的设计，并配备了自动化功能，大大缩短和简化了缝纫过程。所有我们的机器都配备了集成的低功耗电机，环保（生态）并有助于降低维护成本。我们还增加了语音功能，引导您使用所有按键和功能，使使用变得极为简单。虽然这些是计算机化的机器，但无需担心出错——由于有重置按钮，您可以一键恢复出厂设置或保存的首选项。",
  ...
  overlock: {
    ...
    description: {
      ...
      zh: "金星包缝机非常用户友好，得益于语音引导的自动化，它会解释如何解决任何问题或错误。此外，一些型号配备了光电传感器，可以实现全自动缝纫，无需踩踏板——机器处理一切，操作员只需将织物靠近即可。",
  ...
  interlock: {
    ...
    description: {
      ...
      zh: "在我们的套缝机中，我们保持了高标准的自动化和能源效率。这些机器配备了集成电机，可以精确定位针脚，并自动修剪上下线，还有语音引导。此外，我们还实施了高效的油回收系统，防止织物染污。结合空气吹系统，这项创新有效地消除了所有缝纫废物和灰尘。",
  ...
  heavyduty: {
    ...
    description: {
      ...
      zh: "在本部分中，您会找到专为重型任务（如皮革加工）设计的锁缝机。这些机器配备了顶送或三重运输系统和最新的创新技术，包括计算机化语音引导操作、一键快速重置按钮、LED照明、内置750W电机、宽敞的工作区域，以及线修剪器、回针功能和集成压脚升降器等功能。",
  ...
  specialseries: {
    ...
    description: {
      ...
      zh: "本部分包括一系列专门设计的机器，用于缝制各种类型的面料，从轻型到中型到重型。这些自动化机器直观操作，大大减少和简化了缝纫过程。它们提供了多种设置，使您能够完全控制每个操作，但由于集成了扬声器，仍然非常容易操作和调整。",
  ...
  heattransfer: {
    ...
    description: {
      ...
      zh: "热转印系列具有自动恒温系统，可灵活调节压力并提供可靠的性能。该系统包括电子时间控制和过程完成指示器，所有设计都符合人体工学，便于使用和均匀压力分布。它具有电子数字显示屏，用于精确控制温度，精度为（±2°C），并包括微调旋钮。该机器采用铝铸热导率原理，将辐射管和热板集成在一起，确保温度均匀、温度系数低且能效高。该系统特别适合生产奖章、徽章和时尚T恤，兼具经济性和实用性。",
  ...
  needledetector: {
    ...
    description: {
      ...
      zh: "具有高检测灵敏度、卓越的抗干扰能力和稳定的性能。它采用美国RISC高性能芯片，智能、准确地显示断针位置，使其易于定位。功能键集成了机器的整个菜单，增强了信号数据计算，确保操作灵活、方便。它使用红外光电技术来记忆针检测数据，达到100%的准确性。此外，机器在无负载运行10分钟后自动关闭，节约用电并提高安全性。",
  ...
  zigzag: {
    ...
    description: {
      ...
      zh: "新的金星曲折缝机器系列具有创新且吸引人的设计，并配备了自动化功能，大大缩短和简化了缝纫过程。我们所有的机器都配备了环保、低功耗电机，以减少环境影响和维护成本。我们致力于技术进步，已采用尖端功能，如通过数字按钮或触摸面板控制新功能的静音步进电机。",
  ...
  cuttingseries: {
    ...
    description: {
      ...
      zh: "体验每种织物的尖端精度：我们的切割机专为服装行业设计，拥有专门的功能，如集中润滑、精确的曲线切割和自动磨刀。这些多功能工具非常适合从棉花到聚酯材料的高效装订。通过可调节速度和自动停止装置，这些机器确保清洁切割和在广泛的纺织品上的卓越精度，显著提高生产率。",
  ...
  fusingmachine: {
    ...
    description: {
      ...
      zh: "配备了使用上下皮带的直接限位开关的皮带变形防止系统，该机器配有橡胶驱动辊，以确保防滑操作。其单面开放设计允许熔接大于皮带宽度的织物宽度和边缘熔接，而无需全表面热处理。新设计的机身将热量对电气组件的影响降到最低，从而提高了它们的耐用性。此外，MS型号配备了标准旋转剥离装置。",
