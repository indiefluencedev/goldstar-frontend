...
const ModelDetails = ({ addToCompare, compareList }) => {
    ...
    useEffect(() => {
        const fetchModelDetails = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:8000/api/models/${modelType}/${modelId}`);
    ...
    const imageUrl = modelDetails.image ? `http://localhost:8000/${modelDetails.image.replace(/\\/g, '/')}` : '/path/to/default/image.jpg';
