...
const Categories = ({ addToCompare, compareList }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const seriesResponse = await axios.get(`https://testing-backend-s0dg.onrender.comapi/series/${seriesId}`);
                ...
                const modelDetailPromises = seriesData.models.map(async (model) => {
                    const url = `https://testing-backend-s0dg.onrender.comapi/${seriesData.modelType.toLowerCase()}/${model._id}`;
